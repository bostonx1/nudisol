...
3. COMUNIDAD: 5 foros — Presentaciones, Viajes, Vida Naturista, Buzón de Sugerencias y 📰 Noticias (últimas noticias del mundo naturista). Solo usuarios registrados pueden crear temas y responder.
4. ALOJAMIENTOS: Sección activa con alojamientos naturistas verificados. Actualmente disponible el Apartamento Turístico Vera Playa (Almería), en plena urbanización naturista/FKK, a 300 metros de la playa nudista. 1 dormitorio, sofá cama, piscina comunitaria, WiFi, A/C. 130€/noche. Contacto directo con el propietario, sin comisiones. Si alguien tiene un alojamiento naturista y quiere publicarlo gratis, puede escribir a info@nudisol.com
...
El naturismo es respeto al cuerpo, conexión con la naturaleza e igualdad. No tiene relación con el sexo ni el liberalismo. Nudisol es la llave que abre ese mundo.`;
  ...
  function crearUI() {
    ...
    style.textContent = `
      ...
      #solito-mic {
        ...
        border-radius: 50%;
        background: #1F4D52;
        border: 1px solid rgba(242,200,73,0.3);
        cursor: pointer;
        font-size: 16px;
        display: flex;
        align-items: center;
  ...
  async function analizarYGuardar(texto) {
    ...
    const categorias = [];
    const palabrasAlojamiento = ['camping', 'hotel', 'bungalow', 'apartamento', 'alojamiento', 'casa rural', 'hostal', 'cabaña', 'parcela', 'hospedaje', 'dormir', 'quedarme', 'quiero ir', 'reservar'];
    ...
    const provincias = ['almería','cádiz','granada','huelva','málaga','sevilla','córdoba','jaén',
      'alicante','castellón','valencia','barcelona','girona','lleida','tarragona',
      'baleares','canarias','murcia','cantabria','asturias','galicia','navarra',
      'aragón','madrid','toledo','albacete','ciudad real','cuenca','guadalajara',
      'badajoz','cáceres','la rioja','burgos','león','palencia','salamanca',
      'segovia','soria','valladolid','zamora','pontevedra','a coruña','lugo','ourense'];
    const provinciaDetectada = provincias.find(p => textLower.includes(p)) || null;
    if (categorias.length === 0 && !provinciaDetectada) return;
    try {
      const projectId = 'nudisol';
      const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/solito_consultas`;
      await fetch(url, {
        method: 'POST',
  ...
  async function llamarAPI(mensajes) {
    ...
    if (!response.ok) {
      ...
      console.error('Solito API error:', response.status, errData);
  ...
  window.solitoMic = function() {
    if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
      agregarMensaje('bot', 'Tu navegador no soporta el micrófono. Prueba con Chrome. 🌞');
      return;
    }
