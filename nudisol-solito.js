...
A veces recibirás, junto al mensaje del usuario, un bloque marcado como "[DATOS REALES DE NUDISOL...]" con lugares reales de tu base de datos para la zona que preguntan. Si aparece ese bloque, básate SIEMPRE en esos lugares reales para recomendar (menciona su nombre exacto), no inventes otros ni digas que no tienes datos de esa zona. Si no aparece ese bloque, es que no se ha detectado ninguna zona concreta en la pregunta o no hay lugares — en ese caso no inventes nombres de playas, invita a mirar el mapa de la app.`;
  ...
  async function buscarLugaresPorProvincia(provinciaValor) {
  ...
  async function contarLugaresPorProvincia(provinciaValor) {
    const url = 'https://firestore.googleapis.com/v1/projects/nudisol/databases/(default)/documents:runAggregationQuery';
    const body = {
    ...
    try {
      ...
      const data = await res.json();
      return data?.[0]?.result?.aggregateFields?.total?.integerValue
        ? parseInt(data[0].result.aggregateFields.total.integerValue, 10)
        : null;
    } catch (e) {
      return null;
    }
  ...
  async function construirContextoLugares(texto) {
    ...
    const [lugares, total] = await Promise.all([
      buscarLugaresPorProvincia(provinciaValor),
      contarLugaresPorProvincia(provinciaValor)
    ]);
    if (!lugares.length && total === null) return null;
    let bloque = `[DATOS REALES DE NUDISOL para ${provinciaValor}\n`;
    if (total !== null) {
      bloque += `TOTAL EXACTO de lugares verificados en ${provinciaValor}: ${total}. Si el usuario pregunta "cuántas playas/lugares hay", usa este número exacto, no digas "al menos" ni cuentes solo los de la lista de abajo.\n`;
    }
    if (lugares.length) {
      bloque += `Ejemplos reales para recomendar (hay más en la app además de estos, elige el/los que mejor encajen y menciona su nombre exacto, no inventes otros):\n`;
      lugares.forEach(l => {
