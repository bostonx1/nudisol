...
SOBRE NUDISOL:
- App naturista española con +1.000 lugares verificados en TODO el territorio español: península, Canarias (Tenerife, Lanzarote, Gran Canaria, Fuerteventura, La Palma...), Baleares (Mallorca, Menorca, Ibiza, Formentera) y el resto de comunidades autónomas (playas, calas, campings, urbanizaciones)
- IMPORTANTE: Nudisol SÍ tiene cobertura en Canarias y Baleares — nunca digas que solo cubrís la península, sería información falsa
...
3. COMUNIDAD: 7 foros — Vida Naturista, Playas y Alojamientos, Presentaciones, Quedadas y Encuentros, Noticias, Novedades Nudisol (nuevas funciones de la app, solo el equipo abre temas pero todos pueden responder) y Buzón de Sugerencias. Solo usuarios registrados pueden crear temas y responder. Cuando alguien responde a tu publicación, te llega un aviso en la campana 🔔 (arriba a la derecha en Comunidad); al tocarla ves tus notificaciones y puedes ir directo al hilo.
...
DATOS REALES DEL DIRECTORIO:
A veces recibirás, junto al mensaje del usuario, un bloque marcado como "[DATOS REALES DE NUDISOL...]" con lugares reales de tu base de datos para la zona que preguntan. Si aparece ese bloque, básate SIEMPRE en esos lugares reales para recomendar (menciona su nombre exacto), no inventes otros ni digas que no tienes datos de esa zona. Si no aparece ese bloque, es que no se ha detectado ninguna zona concreta en la pregunta o no hay lugares — en ese caso no inventes nombres de playas, invita a mirar el mapa de la app.
CIFRAS Y CANTIDADES: si el bloque de datos reales incluye un "TOTAL", reprodúcelo EXACTAMENTE como está escrito, carácter por carácter. Nunca sustituyas una cifra aproximada ("más de 50") por un número concreto que te parezca más natural o preciso (como "55") — aunque te "suene" más exacto, sería un dato inventado y falso. Ante la duda, copia literalmente el texto proporcionado.`;

  let historial = [];

  // Mapa de palabras que puede usar el usuario -> valor exacto guardado en el campo "provincia" de Firestore
  const MAPA_PROVINCIAS = {
    'tenerife': 'Tenerife', 'lanzarote': 'Lanzarote', 'gran canaria': 'Gran Canaria',
    'fuerteventura': 'Fuerteventura', 'la palma': 'La Palma', 'el hierro': 'El Hierro', 'la gomera': 'La Gomera',
    'almeria': 'Almería', 'almería': 'Almería', 'cadiz': 'Cádiz', 'cádiz': 'Cádiz',
    'malaga': 'Málaga', 'málaga': 'Málaga', 'alicante': 'Alicante', 'girona': 'Girona',
    'tarragona': 'Tarragona', 'barcelona': 'Barcelona', 'castellon': 'Castellón', 'castellón': 'Castellón',
    'murcia': 'Murcia', 'huelva': 'Huelva', 'menorca': 'Menorca', 'mallorca': 'Mallorca',
    'ibiza': 'Ibiza', 'formentera': 'Formentera', 'cantabria': 'Cantabria', 'asturias': 'Asturias',
    'pontevedra': 'Pontevedra', 'a coruna': 'A Coruña', 'a coruña': 'A Coruña', 'lugo': 'Lugo',
    'ourense': 'Ourense', 'madrid': 'Madrid', 'valencia': 'Valencia', 'granada': 'Granada', 'sevilla': 'Sevilla'
  };

  async function buscarLugaresPorProvincia(provinciaValor, limite) {
    const url = 'https://firestore.googleapis.com/v1/projects/nudisol/databases/(default)/documents:runQuery';
    const body = {
      structuredQuery: {
        from: [{ collectionId: 'lugares' }],
        where: {
          fieldFilter: {
            field: { fieldPath: 'provincia' },
            op: 'EQUAL',
            value: { stringValue: provinciaValor }
          }
        },
        limit: limite || 6
      }
      ...
      return (data || [])
        ...
        .map(r => {
          ...
          return {
            ...
            reconocimiento: f.reconocimiento?.stringValue || ''
          };
        })
        .filter(l => l.nombre);
    } catch (e) {
  ...
  async function construirContextoLugares(texto) {
    const textLower = texto.toLowerCase();
    const clave = Object.keys(MAPA_PROVINCIAS).find(k => textLower.includes(k));
    if (!clave) return null;
    const provinciaValor = MAPA_PROVINCIAS[clave];
    const esPreguntaCantidad = /cu[aá]nt[oa]s?/.test(textLower);
    // Si preguntan "cuántas hay", pedimos más de golpe (50) para poder dar un número fiable.
    // Si no, con 6 de ejemplo sobra para recomendar.
    const limite = esPreguntaCantidad ? 50 : 6;
    const lugares = await buscarLugaresPorProvincia(provinciaValor, limite);
    if (!lugares.length) return null;
    let bloque = `[DATOS REALES DE NUDISOL para ${provinciaValor}\n`;
    if (esPreguntaCantidad) {
      const totalTexto = lugares.length >= 50 ? 'más de 50' : String(lugares.length);
      bloque += `TOTAL de lugares verificados en ${provinciaValor}: ${totalTexto}. INSTRUCCIÓN ESTRICTA: usa EXACTAMENTE esta cifra tal cual está escrita aquí ("${totalTexto}"), letra por letra. NO la redondees, NO la sustituyas por un número "más preciso" que te parezca mejor, NO inventes ninguna otra cifra aunque creas recordarla o parecerte más exacta. Si aquí pone "más de 50", tu respuesta debe decir literalmente "más de 50", nunca un número concreto como 55, 60, etc.\n`;
    }
    bloque += `Ejemplos reales para recomendar (elige el/los que mejor encajen y menciona su nombre exacto, no inventes otros):\n`;
    lugares.slice(0, 6).forEach(l => {
      bloque += `- ${l.nombre} (${l.categoria}${l.reconocimiento ? ', ' + l.reconocimiento : ''}): ${l.descripcion}\n`;
    });
    bloque += `]`;
    return bloque;
  }
  ...
  window.solitoEnviar = async function() {
    ...
    try {
      const contextoLugares = await construirContextoLugares(texto);
      let mensajesParaAPI = historial;
      if (contextoLugares) {
        mensajesParaAPI = historial.slice(0, -1).concat([{ role: 'user', content: texto + '\n\n' + contextoLugares }]);
      }
      const respuesta = await llamarAPI(mensajesParaAPI);
      typing.remove();
  ...
  async function llamarAPI(mensajes) {
    ...
    if (!bloqueTexto || !bloqueTexto.text) throw new Error('Respuesta sin texto');
